...
class Ship:
    ...
    def shoot(self):
        ...
        missile_pos = [self.pos[0]+20 + self.radius * self.forward[0], self.pos[1]+40 + self.radius * self.forward[1]]
        
        missile_vel = [self.vel[0] + 4 * self.forward[0], self.vel[1] + 6 * self.forward[1]]
...
def draw(canvas):
    ...
    if started:
        ...
        canvas.blit(splash, (WIDTH//2 - 400, HEIGHT//2 - 350))
    ...
    myfont2 = pygame.font.SysFont("Roboto", 30)
...
def keydown(event):
    ...
    if event.key == K_LEFT:
        Astrography.set_angle_vel(ang_vel)
    if event.key == K_RIGHT:
        Astrography.set_angle_vel(-ang_vel)
    if event.key == K_UP:
        Astrography.set_thrust(True)
...
def keyup(event):
    ...
    if event.key in (K_LEFT,K_RIGHT):
        Astrography.set_angle_vel(0)
    if event.key == K_UP:
        Astrography.set_thrust(False)
...
def rock_spawner():
    ...
    if len(rock_group) < 12 and started:
        ...
        while 1:
            ...
            if dist(random_pos,Astrography.get_position()) <= rock_info.get_radius() + 2 * Astrography.get_radius():
                continue
            else:
                rock_pos = random_pos
                break
...
def click(event):
    global started, lives, score, rock_group, missile_group, Astrography
